...
const registerParticipant = asyncHandler(async (req, res) => {
  ...
  const findUser = await User.findById(id); // Find the student in the DB
  ...
  // Generate the QR code and store it as a file
  const qrCodeImage = await QRCode.toFile(path.join(__dirname, '../../public/temp', `${findUser.email}_qr.png`), JSON.stringify(qrData));
  ...
  const mailOptions = {
    from: '"KLS GIT Admin" <naren.lakamannavar@gmail.com>',  // Sender address
    to: findUser.email,  // Student's email
    subject: 'Your Student QR Code',  // Subject line
    html: `
      <h1>Registration for AURA 2025 successfull!!</h1>
    ...
    attachments: [
      {
        filename: `${findUser.email}_qr.png`,  // Name of the file to be sent
        path: path.join(__dirname, '../../public/temp', `${findUser.email}_qr.png`),  // Path of the file
        cid: 'qrCode'  // This is optional, if you need to reference the image inside the email
      }
    ]
  };
  ...
  try {
    ...
    // Delete the temp QR code file after sending the email
    fs.unlink(path.join(__dirname, '../../public/temp', `${findUser.email}_qr.png`), (err) => {
      if (err) console.error('Error deleting temp QR code file:', err);
    });
  }
});
